...
// Cache the image maps to avoid recreating them on each request
...
async function processBatch(supabase: any, products: any[], batchSize = 10) {
  const batches = []
...
export async function GET() {
  try {
    ...
    // Get all products without images - optimize query to only get what we need
    ...
    // Update products without images in batches
    const updatedCount = await processBatch(supabase, productsWithoutImages || [])

    // Update category images - do this in parallel
    await Promise.all(
      Object.entries(categoryImages).map(([category, imageUrl]) =>
        supabase.from("categories").update({ image_url: imageUrl }).eq("name", category),
      ),
    )
